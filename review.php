<?php
    ...
    $sql = "SELECT japanese.furigana, japanese.kanji, japanese.memorized, japanese.katakana, japanese.english, japanese.id, japanese_img.img_path FROM japanese INNER JOIN japanese_img ON japanese.id = japanese_img.id WHERE memorized = '0' ORDER BY RAND() LIMIT 1;";
    //inner join
...
<body>
    <nav>
        ...
        <form action="/AnkiProject/learning.php" class="mode">
            <label for="toggle-switch">
                <input type="radio" id="toggle-switch" style="color: white;" value="learn" name="btn" onclick="switching()" >
            ...
            <div style="position: relative; padding: 20px 40px; display: flex;">
                <span id="dbwordcounter" style="z-index: -1;">
                    <?php
                ...
                <label for="toggle">
                <input type="radio" id="toggle" style="color: white;" value="review" name="btn" onclick="switching()" checked>
  ...
  if (this.checked) {
    window.location.replace("http://192.168.0.100:8080/learning.php");
  }
})
    </script>
